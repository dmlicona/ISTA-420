...
select custid, orderid, qty,
    ...
    dense rank() over (partition by custid order by qty) as DenseRank
from dbo.orders;
...
Select custid, orderid, qty,
 qty - lag(qty) over(partition by custid order by orderdate, orderid) as X,
 qty - lead(qty) over(partition by custid order by orderdate, orderid) as y,
 from dbo.orders
...
select empid,
    count(orderid) over(partition by empid, year(orderdate) as year2014
